    ...
    def _stage_artifacts(
        ...
        for path in sorted(attempt_dir.rglob("*")):
            ...
            artifact_id = self.preparer.prepare(
                ...
                initiate_idempotency_key=f"{attempt_id}:{identity}",
                lease_token=self.lease_token,
                fencing_token=self.fencing_token,
            )
            item = next(
                item
                for item in self.outbox.due(10000)
                if item.kind == "ARTIFACT_UPLOAD"
                and item.payload["artifact_id"] == str(artifact_id)
            )
...
def _artifact_identity(path: Path) -> tuple[str, str]:
    ...
    return artifact_type, mimetypes.guess_type(path.name)[
        0
