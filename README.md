...
### 平台 Worker（首期 Android）

平台 Worker 使用 Python 3.10，严格串行执行任务。Redis 只接收轻量通知，任务认领、不可变 Plan、Lease、事件、Attempt、Checkpoint 和 Artifact 均通过 Worker API 处理；截图、UI XML、日志和兼容报告 ZIP 会先使用独立 AES-256-GCM DEK 加密，再上传 COS。

1. 复制 `worker-model-profiles.example.yaml` 为 `worker-model-profiles.yaml`，填写本地模型地址和模型名。Plan 只引用 profile 名称，模型 API Key 不写入该文件。
2. 配置运行环境（示例值不是实际凭据）：

```bash
export AUTOGLM_PLATFORM_BASE_URL=http://test-platform.internal
export AUTOGLM_REDIS_URL=redis://redis.internal:6379/0
export AUTOGLM_RUNTIME_ENVIRONMENT=dev
export AUTOGLM_WORKER_ID='replace-with-worker-id-from-platform-resource-page'
export AUTOGLM_WORKER_CREDENTIAL='replace-with-worker-credential'
export AUTOGLM_WORKER_CLAIM_ENABLED=false
export AUTOGLM_MODEL_API_KEY='replace-with-model-credential'
...
# 安装本项目后也可使用：open-autoglm-worker
```
...
spool 基目录必须位于仅 Worker 用户可读写的持久盘。Worker 重启后不会续跑旧手机操作；它会先终止旧子进程并由同环境平台确认旧 Run 终态，在此之前不会扫描设备或领取新任务。完整协议和安全约束见 [Open-AutoGLM 客户端改造方案](docs/Open-AutoGLM客户端改造方案.md)。
