...
def test_duplicate_display_id_uses_distinct_execution_directories(tmp_path):
    ...
    assert all(Path(case.artifacts_dir, "report.json").exists() for case in reporter.cases)
...
def test_multiple_actions_are_appended_to_same_step(tmp_path):
    ...
    report = json.loads(Path(case.artifacts_dir, "report.json").read_text(encoding="utf-8"))
